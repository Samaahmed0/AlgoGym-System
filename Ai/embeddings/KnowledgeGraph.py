...
class KnowledgeGraph:
    """Complete knowledge graph with 20 parents and 155+ children concepts"""
    ...
    def __init__(self):
        ...
        # Build all concepts dict
        self.all_concepts = {}

        # Add parents
        for pid, name in self.parents.items():
            self.all_concepts[pid] = {"name": name, "type": "parent", "difficulty": "Intermediate"}

        # Add children
        for pid, child_list in self.children.items():
            for cid, cname, diff in child_list:
                self.all_concepts[cid] = {"name": cname, "type": "child", "difficulty": diff}
    ...
    def get_triples(self):
        """
        Get all triples as (head, relation_type, tail, weight)
        relation_type 0: parent-child
        relation_type 1: prerequisite
        """
        triples = []

        # Type 0: Parent-child relations
        for pid, children_list in self.children.items():
            for cid, _, _ in children_list:
                triples.append((pid, 0, cid, 1.0))

        # Type 1: Prerequisite relations
        for cid, prereqs in self.prerequisites.items():
            for preid, weight in prereqs:
                if cid in self.all_concepts and preid in self.all_concepts:
                    triples.append((preid, 1, cid, weight))

        return triples

    def get_concept_name(self, cid):
        """Get concept name"""
        if cid in self.all_concepts:
            return self.all_concepts[cid]["name"]
        return None

    def get_prerequisites(self, cid):
        """Get prerequisites for a concept"""
        return self.prerequisites.get(cid, [])

    def print_stats(self):
        """Print KG statistics"""
        triples = self.get_triples()
        parent_child = [t for t in triples if t[1] == 0]
        prerequisites = [t for t in triples if t[1] == 1]

        print("=" * 60)
        print("KNOWLEDGE GRAPH STATISTICS")
        print("=" * 60)
        print(f"Total concepts: {len(self.all_concepts)}")
        print(f"Parent concepts: {len(self.parents)}")
        print(f"Child concepts: {sum(len(c) for c in self.children.values())}")
